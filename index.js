...
const reducedInventors = inventors.reduce(
...
const sortedByYearsLived = [...inventors].sort(
  (young, old) => young.passed - young.year - (old.passed - old.year)
);
...
const reduceByInstances = data.reduce((object, item) => {
  if (!object[item]) {
    object[item] = 0;
  }
  object[item]++;
